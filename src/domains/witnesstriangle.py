...
class TriangleState(Environment):
    """
    This class reprensents a state of a puzzle inspired by the triangle-type puzzle in the game 
    
    The state is represented by several matrices:
        (1) _cells stores the number of triangles in each cell 
    ...
    def get_image_representation(self):
        """
        Generates an image representation for the puzzle. Currently the method supports triangles and includes
        the following channels (third dimension of image): one channel for the triangles; one channel with 1's 
        ...
        entrance of the snake. In total there are 6 different channels. 
        ...
        for j in range(0, self._cells.shape[0]):
            for k in range(0, self._cells.shape[1]):
                if self._cells[j][k] > 0 :
                    image[2*j+1][2*k+1][0] = self._cells[j][k]
        channel_number = 1
...
#             return actions
    ...
    def is_solution(self):
        ...
        (2) the partial path satisfies all the constraints in the puzzle 
        """
